...
Class Juego_arrastrar extends Db{
	...
	function insertar($id,$enunciado,$respuesta){
		try{
			$sql = "INSERT INTO juegos_arrastrar VALUES(:id,:enunciado,:respuesta)";
			...
			$tipo = 'Insertar - Juego Arrastrar';
			$this->debug($error,$tipo);
		}
	}

	function listar_all(){
		$sql_count = "SELECT COUNT(*) AS total FROM juegos_arrastrar";
		...
		$sql = "SELECT * FROM juegos_arrastrar LIMIT :since, :numero";
		$query = $this->db->prepare($sql);
	...
	function listar(){
		...
		$sql = "SELECT * FROM juegos_arrastrar LIMIT :since, :numero";
		$query = $this->db->prepare($sql);
		$this->indice = rand(0,$count - $this->resultadosPorPagina);
        $query->execute(['since' => $this->indice, 'numero' => $this->resultadosPorPagina]);
        if ($query->rowCount() == 0) return False;
		$collect = Null;
		while($row = $query->fetchObject()){
	...
	function listar_load(){
		try{
			$sql_count = "SELECT COUNT(*) AS total FROM juegos_arrastrar left join juegos_progreso ON juegos_arrastrar.id_juego = juegos_progreso.id_juego AND juegos_progreso.nombre_juego = 'juego_arrastrar' WHERE juegos_progreso.id_juego is NULL";
	        $count = $this->db->prepare($sql_count);
	        $count->execute();
	        if ($query->rowCount() == 0) return False;
	        $count = $count->fetch(PDO::FETCH_OBJ)->total; 
			...
			$sql = "SELECT juegos_arrastrar.id_juego, enunciado, respuesta FROM juegos_arrastrar left join juegos_progreso ON juegos_arrastrar.id_juego = juegos_progreso.id_juego AND juegos_progreso.nombre_juego = 'juego_arrastrar' WHERE juegos_progreso.id_juego is NULL LIMIT :since, :numero";
			$query = $this->db->prepare($sql);
	        $query->execute(['since' => $this->indice, 'numero' => $this->resultadosPorPagina]);
	        if ($query->rowCount() == 0) return False;
			...
			if ($collect === Null || count($collect) < 2){
				$sql = "UPDATE USUARIOS SET estado_juego_arrastrar= :estado WHERE usuario = :usuario";
				$query = $this->db->prepare($sql);
				$query->execute(['estado' => 'A', 'usuario' => $_SESSION['usuario']->usuario]);
				if ($query->rowCount() == 0) return False;
				$_SESSION['usuario']->estado_juego_arrastrar = 'A';
			...
			$error = $e->getMessage().' -- '.$e->getCode();
			$tipo = 'Actualizar - Juego Arrastrar';
			$this->debug($error,$tipo);
		}
	}
	...
	function restaurar_completado(){
		try{
			...
			$query = $this->db->prepare($sql);
			$query->execute(['nombre_juego' => 'juego_arrastrar', 'usuario' => $_SESSION['usuario']->usuario]);
			if ($query->rowCount() == 0) return False;
			return True;
		}catch(PDOException $e){
			$error = $e->getMessage().' -- '.$e->getCode();
			$tipo = 'Limpiar - Juego Arrastrar';
			$this->debug($error,$tipo);
		}
	}
	...
	function filtrar($categoria,$busqueda){
        ...
        $sql_count = "SELECT COUNT(*) AS total FROM juegos_arrastrar WHERE titulo like :busqueda";
        $count = $this->db->prepare($sql_count);
		...
		$sql = "SELECT * FROM juegos_arrastrar WHERE titulo like :busqueda LIMIT :since, :numero";
		$query = $this->db->prepare($sql);
	...
	function detalles($id){
		$sql = "SELECT * FROM juegos_arrastrar WHERE id_juego = :id";
		$query = $this->db->prepare($sql);
		$query->execute(['id' => $id]);
		if ($query->rowCount() == 0) return False;
	...
	function actualizar($id,$enunciado,$respuesta){
		try{
			$sql = "UPDATE juegos_arrastrar SET 
			enunciado = :enunciado, 
			respuesta = :respuesta
			WHERE id_juego = :id";
			$query = $this->db->prepare($sql);
	...
	function eliminar($id_juego){
		try{
			$sql = "DELETE FROM juegos_arrastrar WHERE id_juego = :id_juego";
			$query = $this->db->prepare($sql);
